...
def euclidean_loss(logits, labels, weight=1.0, scope=None):
  logits.get_shape().assert_is_compatible_with(labels.get_shape())
  with tf.op_scope([logits, labels], scope, 'EuclideanLoss'):
    _euclidean_loss=0.5*tf.reduce_sum(tf.pow(logits-labels, 2), 1)
    weight = tf.convert_to_tensor(weight,
                                  dtype=logits.dtype.base_dtype,
                                  name='loss_weight')
    loss = weight*tf.reduce_mean(_euclidean_loss)
    tf.add_to_collection(LOSSES_COLLECTION, loss)
    return loss
